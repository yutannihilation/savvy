...
-   Savvy doesn't take care of the output conversion. You have to create a new SEXP object by `Owned...Sxp::new()` and set values by `set_elt()` one by one.
...
After writing more Rust code, you can update the C and R wrapper files by running `savvy-cli update`.

``` sh
savvy-cli update path/to/foo
```

To update the documents, you also have to run `devtools::document()` again.
