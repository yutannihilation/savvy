...
class(`{class_r}`) <- c("{class_r_for_bundle}", "{class_r_sealed}")
...
pub fn generate_r_impl_file(result: &MergedResult, pkg_name: &str) -> String {
    ...
    let r_impls = result
        .impls
        .iter()
        .map(|(ty, i)| generate_r_impl_for_impl(i, ty, &enum_types, pkg_name))
        .collect::<Vec<String>>()
        .join("\n");
...
#' @export
`$<-.{class_r_sealed}` <- function(x, name, value) {{
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}}

#' @export
`[[<-.{class_r_sealed}` <- function(x, i, value) {{
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}}
